...
func _physics_process(delta: float) -> void:
	...
	if button_hit and is_touching_player and Input.is_action_just_pressed("player2attack"):
		#button_hit = false
		get_node("/root/level/Player").animation_taking_damage()
		animation_attack()
		get_node("/root/level/Player").get_damage(DAMAGE_AMOUNT)
...
func get_damage(damage: int) -> void:
	...
	if health < 1:
		...
		get_node("/root/level").player_2_wins()
		die()
